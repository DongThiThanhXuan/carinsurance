    ...
    this.account = this.editAccountForm.value;
      ...
      this.accountService.findById(this.accountid).then(
       ...
       if(this.account.personid.trim() == this.selectaccount.personid.trim() && this.account.accountname.trim() == this.selectaccount.accountname.trim() 
        && this.account.accountemail.trim() == this.selectaccount.accountemail.trim() && this.account.accountadd == this.selectaccount.accountadd
        && this.account.accountphone == this.selectaccount.accountphone){
          this.showWarn();
        }else{
              ...
              if (this.errorPerId === "" && this.errorEmail === "" && this.errorPhone === "" && this.errorName === "") {
                this.accountService.updateAccount(this.account).then(
                  ...
                  if (re.result) {
                    this.reload();
                    this.showSuccess();
                    this.profileComponent.getAccountCustomer()
                    this.closePopup();
                  } else {
                    this.showError();
                  }
